...
import BuyTokenListItem from './BuyTokenListItem'
...
const BuyTokenScreen = () => {
  ...
  const tokens = useMemo((): Ticker[] => {
    return ['SOL']
  }, [])

  const handleTokenItemPress = useCallback(
    (token: Ticker) => () => {
      navigation.navigate('BuyAmountScreen', {
        token,
      })
    },
    [navigation],
  ...
  return (
    <ModalScreen onClose={onClose} title={t('buyScreen.buy')}>
